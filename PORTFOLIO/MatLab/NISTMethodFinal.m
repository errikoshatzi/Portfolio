function NISTMethodFinal()
    ...
    disp('============================================================');
    disp(' SSI MASTER PROGRAM - INTERACTIVE VERSION (WOLF/GIVENS RULE) ');
    disp('============================================================');
    disp('Press ENTER to keep the default value [in brackets].');
    disp('------------------------------------------------------------');
    ...
    F = ask_user('Lateral Design Force F (N)', 441450);
    ...
    B = ask_user('Total Footing Width B (m)', 4.0);
    L = ask_user('Total Footing Length L (m)', 4.5);
    D_emb = ask_user('Embedment Depth D_emb (m)', 0.6);

    disp('--- Damping Parameters ---');
    beta_i = ask_user('Structural Damping beta_i (e.g. 0.05)', 0.05);
    beta_s = ask_user('Soil Hysteretic Damping beta_s (e.g. 0.05)', 0.05);
    ...
    ylabel('Period T (s)', 'FontWeight', 'bold');
    title('Period Elongation due to SSI', 'FontSize', 12);
    legend({'T_{fixed}', 'T_{flex}'}, 'Location', 'northwest');
    grid on; box on;
    ...
    ylabel(ax(1), 'Ratio T_{flex} / T_{fixed}', 'FontWeight', 'bold');
    ylabel(ax(2), 'Total Damping \beta_0', 'FontWeight', 'bold');
    set(ax(1), 'XTick', x, 'XTickLabel', cases, 'FontSize', 10);
    set(ax(2), 'XTick', x, 'XTickLabel', []); 
    title('Elongation and Damping Trends', 'FontSize', 12);
    grid(ax(1), 'on'); 
    print(fig2, fullfile('output','02_ratio_and_damping.png'), '-dpng', '-r300');

    fig3 = figure('Color','w');
    bar(x, delta_cases*1000, 0.5, 'FaceColor', [0.45 0.60 0.90]); 
    set(gca, 'XTick', x, 'XTickLabel', cases, 'FontSize', 10);
    ylabel('Total Top Displacement (mm)', 'FontWeight', 'bold');
    title('SSI Effect on Top Displacement', 'FontSize', 12);
    grid on; box on;
    ...
    set(gca, 'XTick', x, 'XTickLabel', cases, 'FontSize', 10);
    ylabel('Foundation Stiffness', 'FontWeight', 'bold');
    title('Stiffness Reduction per Scenario', 'FontSize', 12);
    legend({'Horizontal (K_x)', 'Rocking (K_{yy})'}, 'Location', 'northeast');
    grid on; box on;
    print(fig4, fullfile('output','04_stiffness_trends.png'), '-dpng', '-r300');

    disp('============================================================');
    disp('COMPLETED! 4 plots saved in "output" folder.');
    disp('============================================================');
end
